...
var granule = ee
  .Image("LANDSAT/LC08/C01/T1_SR/LC08_105014_20190819")
  .select(["B1", "B2", "B3", "B4", "B5", "B6", "B7", "B10", "B11"]);

//var geometry = ee.FeatureCollection("users/muravevtsnigri/Num_Magadan_GEE");
/*var collection = ee.ImageCollection('LANDSAT/LC08/C01/T1_SR') // searches all sentinel 2 imagery pixels...
...
Map.addLayer(
  image,
  { bands: ["B7", "B5", "B3"], min: 6000, max: 12000 },
  "Composite1"
);
...
/*Export.image.toDrive({
    image: dcs(image, geometry, 1000).select(rgbdcs),
    description: 'DCS',
    scale: 30,
    region: geometry,
    maxPixels: 1e9,
